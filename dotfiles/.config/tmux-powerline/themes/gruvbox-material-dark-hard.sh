...
if [ -z "$TMUX_POWERLINE_LEFT_STATUS_SEGMENTS" ]; then
	TMUX_POWERLINE_LEFT_STATUS_SEGMENTS=(
		"tmux_session_info 208 234"
		# Windows are handled by tmux's window-status-format, not as a segment
	)
fi
...
if [ -z "$TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS" ]; then
	TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS=(
        "time 235 223 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
		"date 235 223 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
	)
fi
